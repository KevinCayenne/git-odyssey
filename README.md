...
| --- | --- |
| `/` | 卷首。整站的論點和目錄 |
| `/start` | 入門。給完全沒碰過 git 的人：八步走完第一個 commit、六個詞 |
| `/flow` | 指令流程圖。22 個常用指令，每個一張會動的圖 |
| `/play` | 沙盒。五種開場，隨便亂搞 |
...
那頁中間是一個八步走查：按「下一步」就好，不用打字，但每一步都是真的餵進引擎跑的指令 ——
所以三格的變化跟學生自己動手打出來的一模一樣。投影著按，邊按邊講，大約十五分鐘。

步驟寫在 `src/lib/quests/primer.ts`，**測試會整串跑一遍**並逐步比對三格的狀態，
所以解說跟畫面不可能對不上；`npm run e2e` 另外用真瀏覽器把八步點過一遍。

### 忘記某個指令在幹嘛就翻 `/flow`

那頁把 git 拆成六個「東西可以待的地方」——工作目錄、暫存區、本地歷史、
origin/\*、遠端，外加一個口袋——然後把每個常用指令畫成一段在這些地方之間的搬運。
按播放就看得到東西在跑；會動到歷史的指令（commit、merge、rebase、revert…）
底下還有一張前後內插的小圖，rebase 那幾顆 commit 是真的會飄到新位置。

順序本身就是知識：**往右是把東西交出去，往左是把東西拿回來**。

資料寫在 `src/lib/quests/flows.ts`，每一條都附一段真的餵進引擎的腳本。
測試會跑過一遍，然後把引擎實際改到的格子跟圖上宣告的箭頭**逐項比對** ——
畫了一條指向某一格的箭頭、但引擎根本沒動那一格，CI 就不會過。
旁白講的形狀也一起釘住：merge 那張如果退化成 fast-forward，
「收成一個合流點」那句話就變成謊話，測試會抓到。
...
src/app/              路由（卷首、入門、指令圖、沙盒、關卡、人與 AI、生活流、講師）
```
...
`flows.test.ts` 對每個指令做三件事：把示範腳本跑一遍、
把 `observePlaces(before, after)` 算出來的「引擎真的改到哪幾格」
跟資料裡宣告的 `touches` 對到完全相等、再確認每一條畫出來的箭頭
終點都落在真的被改到的格子上。

所以這頁不會有「圖上畫著箭頭、實際上什麼都沒發生」這種事 ——
那正是市面上大部分 git 圖解最容易出錯的地方。
...
- 中文段落用首行縮排接續，不是空行
- **字級全部是 rem，根字級 112.5%（18px）**。要整站放大縮小只要動 `html { font-size }`
  一行；也因為不是 px，使用者在瀏覽器把字調大時版面真的會回應
